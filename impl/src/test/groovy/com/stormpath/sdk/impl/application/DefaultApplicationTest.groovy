...
class DefaultApplicationTest {
    ...
    void testResendEmailVerificationToken() {
        ...
        def internalDataStore = createStrictMock(InternalDataStore)
        def defaultApplication = new DefaultApplication(internalDataStore, properties)
        VerificationEmailRequest verificationEmailRequest = createStrictMock(VerificationEmailRequest)
        def accountStore = createStrictMock(AccountStore)
        ...
        expect(accountStore.getHref()).andReturn(accountStoreHref)
        expect(internalDataStore.create(defaultApplication.getHref() + "/verificationEmails", verificationEmailRequest, VerificationEmailRequest.class)).andReturn(null)
