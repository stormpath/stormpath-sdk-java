 ...
 * @since 1.0.0
 */
class DefaultLinkedInCreateProviderRequestBuilderTest {

    @Test
    void test() {
        def providerRequest = Providers.LINKEDIN;
        def requestBuilder = providerRequest.builder();
        assertTrue(requestBuilder instanceof LinkedInCreateProviderRequestBuilder)
        ...
        assertEquals(request.getProvider().getProviderId(), "linkedin")
        def provider = request.getProvider()
        assertTrue(provider instanceof DefaultLinkedInProvider)
        provider = (DefaultLinkedInProvider) provider
    ...
    void testMissingAllProperties() {
        def requestBuilder = Providers.LINKEDIN.builder();
