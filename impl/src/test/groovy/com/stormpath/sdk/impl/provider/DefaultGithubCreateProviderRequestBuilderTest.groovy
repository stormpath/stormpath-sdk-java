 ...
 * @since 1.0.0
 */
class DefaultGithubCreateProviderRequestBuilderTest {

    @Test
    void test() {
        def providerRequest = Providers.GITHUB;
        def requestBuilder = providerRequest.builder();
        assertTrue(requestBuilder instanceof GithubCreateProviderRequestBuilder)
        ...
        assertEquals(request.getProvider().getProviderId(), "github")
        def provider = request.getProvider()
        assertTrue(provider instanceof DefaultGithubProvider)
        provider = (DefaultGithubProvider) provider
    ...
    void testMissingAllProperties() {
        def requestBuilder = Providers.GITHUB.builder();
