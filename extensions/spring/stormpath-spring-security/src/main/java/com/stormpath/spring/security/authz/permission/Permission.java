 ...
 * <p/>
 * Spring Security is <em>role-based</em> in nature, where a role represents common behavior for certain user types.
 * For example, a system might have an <em>Aministrator</em> role, a <em>User</em> or <em>Guest</em> roles, etc.
 * <p/>
 ...
 * model.
 * <p/>
 * In this case, we extend the {@link GrantedAuthority} interface with a method that allows us to compare permissions.
 * With such operation we can customize the way a comparison is made. For example, instead of comparing if one permission
 * equals a required permission, we can check if one permission satisfies it in some other way.
 * </p>
 * A benefit to this is that, although it assumes most systems are based on these types of static role or
 * dynamic role w/ permission schemes, it does not require a system to model their security data this way - all
 * Permission checks can been relegated to custom implementations, and only those implementations really determine how
 * a user 'has' a permission or not.  The {@link org.springframework.security.authentication.AuthenticationProvider
 * AuthenticationProvider} could use the semantics described here, or it could utilize some other mechanism entirely - it
 * is always up to the application developer.
 * <p/>
 * Here we provide a very powerful default implementation of this interface in the form of the
 * {@link com.stormpath.spring.security.authz.permission.WildcardPermission WildcardPermission}. We highly recommend that you
 * investigate this class before trying to implement your own <code>Permission</code>s.
 *
 * @see com.stormpath.spring.security.authz.permission.WildcardPermission WildcardPermission
 * @since 0.1.1
 */
